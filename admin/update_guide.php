...
if($dopost=='test')
{
    ...
    {
        $offUrl = SpGetNewInfo();
        echo "<div class='query-btn ue-clear'><b class='clear'>你系统版本最后更新时间为：{$oktime}，当前没有可用的更新</b>\r\n";
        echo "<iframe name='stafrm' src='{$offUrl}&uptime={$oktime}' frameborder='0' id='stafrm' width='98%' height='50'></iframe></div>";
    }
    else
    {
        if ('no' == $auto) {
            $offUrl = SpGetNewInfo();
            echo "<div class='query-btn ue-clear'><b>你系统版本最后更新时间为：{$oktime} </b><br/><a href='javascript:LoadUpdateInfos();' class='confirm'>进行在线更新</a><a href='sys_repair.php' class='clear'>系统错误修复</a>";
            echo "<iframe name='stafrm' src='{$offUrl}&uptime={$oktime}' frameborder='0' id='stafrm' width='98%' height='50'></iframe></div>";
            exit();
        }
...
else if($dopost=='down')
{
    ...
    else
    {
        ...
        foreach($sqls as $sql)
        {
            ...
            $dhd->OpenUrl($downfile);
            // 请求成功才拼接 by 小虎哥
            if (200 == intval($dhd->m_httphead['http-code'])) { 
                $ct .= $dhd->GetHtml();
            }
            // --end
        }
